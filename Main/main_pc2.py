...
def receive_robot_data(_,info_computer_share):
...
def GUI_loop(_,info_computer_share):
...
def main():
	...
	info_computer_share = {'state': -1, 'last_bisturi_pos': [0,0,0,0,0],  'cutting_plan':[0,0], 'coliding':False }
							...
							#		3 - Cutting
							#		4 - Finished running
	graphical_interface = threading.Thread(target=GUI_loop, args=(_,info_computer_share))
	receive_data_server = threading.Thread(target=receive_robot_data,args=(_,info_computer_share))
